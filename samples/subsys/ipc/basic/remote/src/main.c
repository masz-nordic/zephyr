 ...
 */
int send_to_host(const unsigned char *const restrict buffer, const unsigned int size)
{
	/* Try and get lock */
 ...
 */
int read_from_host(unsigned char *const restrict buffer, unsigned int size)
{
	/* Try and get lock */
			  ...
			  struct mbox_msg *msg_data)
{
	...
	int bytes_all = host_has_data();

	if (bytes_all > 0) {
		int bytes_read = read_from_host((unsigned char *const restrict)&data_packet,
						sizeof(data_packet));
		if (bytes_read < 0) {
			LOG_ERR("Failed to read from host");
		} else {
			LOG_INF("Read %d bytes from host, received %d", bytes_read, bytes_all);
		}
	} else {
		LOG_INF("No data from host");
	}
}
...
int main(void)
{
	...
	/* Send signal only to other core */
	...
	/* Send struct with data to other core */
	while (bytes_sent <= 0) {
		bytes_sent = send_to_host((const unsigned char *const restrict)&data_packet,
					  sizeof(data_packet));
		if (bytes_sent < 0) {
	...
	LOG_INF("Basic core-communication REMOTE demo ended");
