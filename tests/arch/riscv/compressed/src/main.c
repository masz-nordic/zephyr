...
/* Factored out to a separate function to avoid compiler error:
 * "impossible constraint" due to limited amount of registers.
 */
ZTEST(riscv_compressed, test_zca_integer_sp)
{
	unsigned long a = 0;
	unsigned long b = 0;
	unsigned long * c = &a;
