					 ...
					 gpio_port_value_t value)
{
	...
	nrfe_gpio_data_packet_t msg = {.opcode = NRFE_GPIO_PIN_SET,
				       .pin = set_mask,
				       .port = get_port_cfg(port)->port_num};
...
{
	nrfe_gpio_data_packet_t msg = {.opcode = NRFE_GPIO_PIN_CLEAR,
				       .pin = mask,
				       .port = get_port_cfg(port)->port_num};
...
{
	nrfe_gpio_data_packet_t msg = {.opcode = NRFE_GPIO_PIN_TOGGLE,
				       .pin = mask,
				       .port = get_port_cfg(port)->port_num};
...
#define GPIO_NRFE_DEVICE(id)                                                                       \
	...
	DEVICE_DT_INST_DEFINE(id, gpio_nrfe_init, NULL, &gpio_nrfe_p##id##_data,                   \
			      &gpio_nrfe_p##id##_cfg, POST_KERNEL, CONFIG_GPIO_INIT_PRIORITY,      \
